...
namespace TechRental.views.panels
{
    public partial class admin_inventory : UserControl
    {
        ...
        private void btnApply_Click(object sender, EventArgs e)
        {
            ...
            {
                UpdatePaginatedData(FilterEquipments());

                //dgvEquipment.DataSource = FilterEquipments();
            }
        ...
        {
            if (cbColumn.SelectedItem == null) return;
        ...
        private Control CreateControlForType(string type, string col)
        {
            return type switch
            {
                ...
                "Boolean" => new ComboBox
                {
                    Name = $"{col}_boolean",
                    Dock = DockStyle.Fill,
                    DataSource = new BindingSource(new Dictionary<string, bool> { { "True", true }, { "False", false } }, null),
                    DisplayMember = "Key",
                    ValueMember = "Value"
                },
                "Decimal" => new NumericUpDown
                {
                    Name = $"{col}_decimal",
                    Dock = DockStyle.Fill,
                    Minimum = 0.01M,
                ...
                "" => new Panel(),
                _ => CreateReferenceDropdown(col, type)
            };
        }
